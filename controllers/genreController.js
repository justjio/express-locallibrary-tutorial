...
//Import the functions for validation and sanitization
...
exports.genre_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (results.genre==null) { // No results.
            var err = new Error('I no see am ooo!');
            err.status = 404;
            return next(err);
        }
        // Successful, so render
...
exports.genre_create_post =  [
    ...
    // Validate that the name field is not empty and has only alphabets
    body('name').isLength({ min: 1 }).trim().withMessage('Genre name required.').isAlpha().withMessage('Genre name can only be alphabet letters.'),
